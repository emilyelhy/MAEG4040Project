...
int getBLK(uint16_t camARR[]) {
  ...
  if ((right - left) > 40) {
    if (left < 14 && right > 114) return -10;
    else if (left < 14) return -3;
    else if (right > 114) return -4;
  }
  else return left;
}
...
void tracking_algoritm()
{
  ...
  */
  ...
  if (pos == -1){
    ...
    motorMOVE(CW, NSL, NSR);
  else if (pos == -4)
    motorMOVE(CCW, NSL, NSR);
  else if (pos <= 24)
    motorMOVE(CW, 34 - k * pos, 32 - k * pos);
  else if (pos >= 104)
    motorMOVE(CCW, k * pos - 11.696, k * pos - 13.696);
  else if (pos > 24 && pos < 44)
    motorMOVE(FW, NSL-17, NSR-5);
  else if (pos > 84 && pos < 104)
    motorMOVE(FW, NSL-5, NSR-17);
  else
    motorMOVE(FW, NSL, NSR);
